...
import { Terminal, Code, Copy, Check, ExternalLink, Cpu, BookOpen, Layers } from 'lucide-react';
...
try:
    result = core.launch(
        LaunchRequest(
            ...
            quick_play_singleplayer="New World", # Vào thẳng world
        )
    )
...
# Tạo một instance NeoForge mới với RAM và cờ JVM tối ưu
instance = core.instances.create(
    InstanceCreateRequest(
        name="NeoForge 1.21.1 Cyber",
        version_id="1.21.1",
...
# Khởi chạy một instance bất kỳ không cần mở giao diện:
mcw-core-launch --root ./mcw-data --instance "NeoForge 1.21.1 Cyber" --username Player`
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="px-5 py-2.5 rounded-xl font-semibold text-xs tracking-wider bg-obsidian-850 hover:bg-obsidian-800 text-slate-200 border border-white/10 hover:border-cyan-500/40 flex items-center gap-2 transition-all"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="px-5 py-2.5 rounded-xl font-semibold text-xs tracking-wider bg-obsidian-950 hover:bg-obsidian-900 text-slate-300 border border-white/10 flex items-center gap-2 transition-all"
              >
          ...
          <div className="lg:col-span-7 glass-panel rounded-2xl border border-white/10 shadow-2xl overflow-hidden">
            {/* Tab header */}
            <div className="bg-obsidian-950 px-4 py-3 flex items-center justify-between border-b border-white/10">
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setActiveTab('launch')}
                  className={`px-3 py-1.5 rounded-lg text-xs font-mono font-medium transition-colors ${
                    activeTab === 'launch'
                      ? 'bg-obsidian-800 text-cyber-cyan border border-cyan-500/30'
              ...
              <button
                onClick={handleCopy}
                className="flex items-center gap-1.5 text-xs font-mono text-slate-400 hover:text-white bg-white/5 hover:bg-white/10 px-2.5 py-1 rounded transition-colors"
              >
